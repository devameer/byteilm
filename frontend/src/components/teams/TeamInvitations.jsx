...
const TeamInvitations = ({ teamId, teamName }) => {
    ...
    const getStatusBadge = (invitation) => {
        if (invitation.revoked_at) {
            return <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">ملغاة</span>;
        }
        if (invitation.expires_at && new Date(invitation.expires_at) < new Date()) {
            return <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-bold">منتهية</span>;
        }
        if (invitation.max_uses && invitation.uses_count >= invitation.max_uses) {
            return <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-bold">مستنفذة</span>;
        }
        return <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">نشطة</span>;
    };
    ...
    return (
        <div className="bg-white rounded-2xl p-6 border-2 border-slate-200 shadow-lg">
            ...
            {showCreateForm && (
                <form onSubmit={createInvitation} className="mb-6 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-4 border-2 border-indigo-200">
                    <div className="grid gap-4 md:grid-cols-3">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">
                                <i className="fas fa-user-tag ml-1 text-slate-400"></i>
                                الدور
                            </label>
                            <select
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">
                                <i className="fas fa-calendar-alt ml-1 text-slate-400"></i>
                                صلاحية الرابط (أيام)
                            </label>
                            <input
                                type="number"
                                min="1"
                                max="30"
                                className="form-input w-full border-2 border-slate-200 rounded-lg px-3 py-2"
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">
                                <i className="fas fa-hashtag ml-1 text-slate-400"></i>
                                عدد الاستخدامات
                            </label>
                            <input
                                type="number"
            ...
            {loading ? (
                ...
                <div className="text-center py-12 bg-slate-50 rounded-xl">
                    ...
                    <p className="text-slate-500">لم يتم إنشاء أي روابط دعوة بعد</p>
                    <p className="text-xs text-slate-400 mt-1">أنشئ رابط دعوة لدعوة أعضاء جدد للفريق</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {invitations.map((invitation) => (
                        <div
                            key={invitation.id}
                            className="border-2 border-slate-200 rounded-xl p-4 hover:border-indigo-300 hover:shadow-md transition-all"
                        >
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="bg-slate-50 rounded-lg p-3 mb-2 font-mono text-sm text-slate-700 break-all border border-slate-200">
